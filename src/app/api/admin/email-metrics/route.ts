...
export async function GET(req: Request) {
  ...
  try {
    ...
    const totalDelivered = eventCounts.find(e => e.type === 'delivered')?._count.id || 0;
    const totalOpened = eventCounts.find(e => e.type === 'opened')?._count.id || 0;
    const totalClicked = eventCounts.find(e => e.type === 'clicked')?._count.id || 0;
    const totalBounced = eventCounts.find(e => e.type === 'bounced')?._count.id || 0;
    const totalComplained = eventCounts.find(e => e.type === 'complained')?._count.id || 0;
